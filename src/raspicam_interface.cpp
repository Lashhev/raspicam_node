...
RaspicamInterface::RaspicamInterface(RASPICAM_INPUT_DATA& input_parameters):
skip_frames(0),
camera_video_port(nullptr),
camera_preview_port(nullptr),
splitter_input_port(nullptr),
splitter_output_enc(nullptr),
splitter_output_raw(nullptr),
image_encoder_input_port(nullptr),
image_encoder_output_port(nullptr),
video_encoder_input_port(nullptr),
video_encoder_output_port(nullptr),
callback_data_enc(nullptr),
h264_callback_data_enc(nullptr),
callback_data_raw(nullptr)
{
    state.input_parameters = input_parameters;
}

RaspicamInterface::~RaspicamInterface()
{
 ...
 *
 */
MMAL_COMPONENT_T* RaspicamInterface::create_camera_component()
{
...
//   state.camera_parameters.stereo_mode.mode = stereo_mode_from_string("tb");
  ...
  if (!camera->output_num) {
    ...
    LOG("Camera doesn't have output ports\n");
    goto error;
  }
  ...
  //  set up the camera configuration
  {
    ...
    cam_config.max_stills_w = state.input_parameters.width;
    cam_config.max_stills_h = state.input_parameters.height;
    cam_config.stills_yuv422 = 0;
    cam_config.one_shot_stills = 0;
    cam_config.max_preview_video_w = state.input_parameters.width;
    cam_config.max_preview_video_h = state.input_parameters.height;
  ...
  // Select the camera to use
  {
    ...
    camera_num.value = state.input_parameters.camera_id;

    status = mmal_port_parameter_set(camera->control, &camera_num.hdr);
    if (status != MMAL_SUCCESS) {
      printf("Could not select camera : error %d\n", status);
      goto error;
    }
  }
  ...
  format->es->video.width = state.input_parameters.width;
  format->es->video.height = state.input_parameters.height;
  format->es->video.crop.x = 0;
  format->es->video.crop.y = 0;
  format->es->video.crop.width = state.input_parameters.width;
  format->es->video.crop.height = state.input_parameters.height;
  format->es->video.frame_rate.num = state.input_parameters.framerate;
  ...
  if (status) {
    ...
    printf("camera preview format couldn't be set\n");
    goto error;
  }
 ...
 *
 */
MMAL_STATUS_T RaspicamInterface::create_image_encoder_component()
{
  ...
  if (status != MMAL_SUCCESS) {
    ...
    printf("Unable to create image encoder component\n");
    goto error;
  }
  ...
  if (!encoder->input_num || !encoder->output_num) {
    ...
    printf("Image encoder doesn't have input/output ports\n");
    goto error;
  }
  ...
  if (status != MMAL_SUCCESS) {
    ...
    printf("Unable to set format on image encoder output port\n");
    goto error;
  }

  // Set the JPEG quality level
  status = mmal_port_parameter_set_uint32(encoder_output, MMAL_PARAMETER_JPEG_Q_FACTOR, state.input_parameters.quality);
  ...
  if (status != MMAL_SUCCESS) {
    ...
    printf("Unable to enable image encoder component\n");
    goto error;
  }
  ...
  if (!pool) {
    ...
    printf("Failed to create buffer header pool for image encoder output port %s\n", encoder_output->name);
  }
  ...
  printf("Image encoder component done\n");
 ...
 *
 */
MMAL_STATUS_T RaspicamInterface::create_video_encoder_component()
{
  MMAL_COMPONENT_T* encoder = 0;
  MMAL_PORT_T *encoder_input = nullptr, *encoder_output = nullptr;
  MMAL_STATUS_T status;
  ...
  if (status != MMAL_SUCCESS) {
    ...
    printf("Unable to set H264 profile on video encoder output port\n");
    goto error;
  }
  ...
  if (!pool) {
    ...
    printf("Failed to create buffer header pool for video encoder output port %s\n", encoder_output->name);
  }
  ...
  printf("Video encoder component done\n");
 ...
 *
 */
MMAL_STATUS_T RaspicamInterface::create_splitter_component()
{
  MMAL_COMPONENT_T* splitter = 0;
  MMAL_PORT_T* splitter_input = nullptr;
  splitter_output_enc = nullptr, splitter_output_raw = nullptr;
  MMAL_STATUS_T status;
  MMAL_POOL_T* pool;
  MMAL_ES_FORMAT_T* format;
  ...
  if (!splitter->input_num) {
    status = MMAL_ENOSYS;
    printf("Video splitter doesn't have input ports\n");
    goto error;
  }

  if (splitter->output_num < 2) {
    status = MMAL_ENOSYS;
    printf("Video splitter doesn't have enough output ports\n");
    goto error;
  }
  ...
  if (status != MMAL_SUCCESS) {
    ...
    printf("Unable to set format on splitter input port\n");
    goto error;
  }
  ...
  if (status != MMAL_SUCCESS) {
    ...
    printf("Unable to enable splitter component\n");
    goto error;
  }
  ...
  printf("splitter component done\n");
 ...
 *
 */
MMAL_STATUS_T RaspicamInterface::connect_ports(MMAL_PORT_T* output_port, 
 ...
 */
int RaspicamInterface::init_cam()
{
  MMAL_STATUS_T status;
  ...
  if (!create_camera_component()) {
    printf("%s: Failed to create camera component\n", __func__);
  } else if ((status = create_image_encoder_component()) != MMAL_SUCCESS) {
    printf("%s: Failed to create image encoder component\n", __func__);
    state.camera_component.reset(nullptr);
  } else if ((status = create_video_encoder_component()) != MMAL_SUCCESS) {
    printf("%s: Failed to create H264 encoder component\n", __func__);
    state.image_encoder_component.reset(nullptr);
    state.camera_component.reset(nullptr);
  } else if ((status = create_splitter_component()) != MMAL_SUCCESS) {
    printf("%s: Failed to create splitter component\n", __func__);
    state.image_encoder_component.reset(nullptr);
    ...
    if (status != MMAL_SUCCESS) {
      printf("%s: Failed to connect camera video port to splitter input\n", __func__);
      return 1;
    }

    status = connect_ports(splitter_output_enc, image_encoder_input_port, state.image_encoder_connection);
    if (status != MMAL_SUCCESS) {
      printf("%s: Failed to connect camera splitter port to image encoder input\n", __func__);
      return 1;
    }

    image_encoder_output_port = state.image_encoder_component->output[0];

    callback_data_enc = new PORT_USERDATA(state);
    ...
    status = mmal_port_enable(image_encoder_output_port, (MMAL_PORT_BH_CB_T)&RaspicamInterface::image_encoder_buffer_callback);
    if (status != MMAL_SUCCESS) {
      printf("Failed to setup image encoder output\n");
      return 1;
    }
    ...
    if (state.input_parameters.enable_imv_pub) {
      ...
      if (status != MMAL_SUCCESS) {
        printf("%s: Failed to connect camera preview port to encoder input\n", __func__);
        return 1;
      }

      video_encoder_output_port = state.video_encoder_component->output[0];
      h264_callback_data_enc = new PORT_USERDATA(state);
      // Set up our userdata - this is passed though to the callback where we
      // need the information.
      video_encoder_output_port->userdata = h264_callback_data_enc;
      // Enable the encoder output port and tell it its callback function
      status = mmal_port_enable(video_encoder_output_port, (MMAL_PORT_BH_CB_T)&RaspicamInterface::video_encoder_buffer_callback);
      if (status != MMAL_SUCCESS) {
        printf("Failed to setup video encoder output\n");
        return 1;
      }
    }
    ...
    if (state.input_parameters.enable_raw_pub) {
      ...
      callback_data_raw = new PORT_USERDATA(state);
      ...
      status = mmal_port_enable(splitter_output_raw, (MMAL_PORT_BH_CB_T)&RaspicamInterface::splitter_buffer_callback);
      if (status != MMAL_SUCCESS) {
        printf("Failed to setup splitter output");
        return 1;
      }
    }
    state.input_parameters.isInit = true;
  }
  return 0;
}
/**
 ...
 */
void RaspicamInterface::image_encoder_buffer_callback(MMAL_PORT_T* port, MMAL_BUFFER_HEADER_T* buffer)
{
  // We pass our own memory and other stuff in via the userdata field.
  PORT_USERDATA* pData = port->userdata;
  printf("%p\n", port->userdata->pstate);
  if (pData && pData->pstate.input_parameters.isInit) {
    size_t bytes_written = buffer->length;
    if (buffer->length) {
      if (pData->id != INT_MAX) {
        if (pData->id + buffer->length > IMG_BUFFER_SIZE) {
          printf("pData->id (%d) + buffer->length (%d) > "
    ...
    if (complete) {
      if (pData->id != INT_MAX) {
        ...
        if (skip_frames > 0 && pData->frames_skipped < skip_frames) {
          ...
          image_compressed_out(pData, buffer);
          pData->frame++;

	//   // Update diagnosics if needed
	//   pData->pstate.updater.update();// TODO: do not forget to 
        }
      }
      pData->id = 0;
    }
  ...
  if (port->is_enabled) {
    ...
    if (!new_buffer || status != MMAL_SUCCESS) {
      ...
      printf("Unable to return a buffer to the image encoder port\n");
    }
  }
}
/**
 ...
 * @param buffer mmal buffer header pointer
 */
void RaspicamInterface::video_encoder_buffer_callback(MMAL_PORT_T* port, MMAL_BUFFER_HEADER_T* buffer)
{
  // We pass our own memory and other stuff in via the userdata field.
  PORT_USERDATA* pData = port->userdata;
  ...
  if (buffer->flags & MMAL_BUFFER_HEADER_FLAG_CODECSIDEINFO) {
    mmal_buffer_header_mem_lock(buffer);
    video_encoded_out(pData, buffer);
    mmal_buffer_header_mem_unlock(buffer);
    pData->frame++;
  }
  ...
  if (port->is_enabled) {
    ...
    if (!new_buffer || status != MMAL_SUCCESS) {
      ...
      printf("Unable to return a buffer to the video encoder port\n");
    }
  }
}

void RaspicamInterface::splitter_buffer_callback(MMAL_PORT_T* port, MMAL_BUFFER_HEADER_T* buffer)
{
  ...
  if (port->is_enabled) {
    ...
    if (!new_buffer || status != MMAL_SUCCESS) {
      ...
      printf("Unable to return a buffer to the splitter port\n");
    }
  }  
}

int RaspicamInterface::start_capture()
{
  if (!(state.input_parameters.isInit))
    printf("Tried to start capture before camera is inited\n");

  camera_video_port = state.camera_component->output[mmal::camera_port::video];
  image_encoder_output_port = state.image_encoder_component->output[0];
  video_encoder_output_port = state.video_encoder_component->output[0];
  splitter_output_raw = state.splitter_component->output[1];
  printf("Starting video capture (%d, %d, %d, %d)\n", state.input_parameters.width, state.input_parameters.height, state.input_parameters.quality, state.input_parameters.framerate);
  ...
  // Send all the buffers to the image encoder output port
  {
    ...
    for (q = 0; q < num; q++) {
      ...
      if (mmal_port_send_buffer(image_encoder_output_port, buffer) != MMAL_SUCCESS) {
        ...
        printf("Unable to send a buffer to image encoder output port (%d)\n", q);
      }
    }
  }
  ...
  if (state.input_parameters.enable_imv_pub) {
    ...
    for (q = 0; q < num; q++) {
      ...
      if (mmal_port_send_buffer(video_encoder_output_port, buffer) != MMAL_SUCCESS) {
        ...
        printf("Unable to send a buffer to video encoder output port (%d)\n", q);
      }
    }
  }
  ...
  if (state.input_parameters.enable_raw_pub) {
    ...
    for (q = 0; q < num; q++) {
      ...
      if (mmal_port_send_buffer(splitter_output_raw, buffer) != MMAL_SUCCESS) {
        ...
        printf("Unable to send a buffer to splitter output port (%d)\n", q);
      }
    }
  }
...
int RaspicamInterface::close_cam()
{
    if (state.input_parameters.isInit) {
